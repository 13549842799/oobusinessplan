...
export default {
  ...
  created () {
    let v = this
    v.page = v.createPage({url: rolesApi.listUrl(), pageSize: 8, total: 8})
  },
  computed: {},
  methods: {
    openAddDialog () {
      this.$refs.roleForm.initAddForm()
    },
    openEditDialog () {
      if (!this.current) {
      ...
      this.$refs.roleForm.initEditForm(this.current)
    },
    deleteRoles () {
      let v = this
      const delMethod = () => {
        rolesApi.deleteRoles(v.current.id).then(res => {
          v.page.searchPage()
